...
{
    /**
     * @var string that is default country in ISO 3166-1 alpha-2 format
     */
    ...
    public function validate(string $input, array $rule, $data)
    {
        ...
        if (is_string($data)) {
            if (false !== strpos($data, '.')) {
                ...
                if (is_numeric($code)) {
                    ...
                    if ($relatedCountries and 1 == count($relatedCountries)) {
        ...
        switch ($data['code']) {
             ...
             */
            case 'IR':
                if (!Safe::is_cellphone_ir(('98' !== substr($data['number'], 0, 2) ? '98' : '').$data['number'])) {
